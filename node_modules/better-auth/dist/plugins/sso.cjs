...
var sso = (options) => {
  return {
    id: "sso",
    endpoints: {
      createOIDCProvider: chunkCQNPMJJ3_cjs.createAuthEndpoint(
        "/sso/register",
        {
          method: "POST",
          body: zod.z.object({
      ...
      signInSSO: chunkCQNPMJJ3_cjs.createAuthEndpoint(
        "/sign-in/sso",
        {
          method: "POST",
          body: zod.z.object({
        ...
        async (ctx) => {
          ...
          const authorizationURL = await chunkV6RGXSG4_cjs.createAuthorizationURL({
            id: provider.issuer,
            options: {
      ...
      callbackSSO: chunkCQNPMJJ3_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const tokenResponse = await chunkV6RGXSG4_cjs.validateAuthorizationCode({
            code,
          ...
          if (tokenResponse.idToken) {
            ...
            const verified = await chunkV6RGXSG4_cjs.validateToken(
          ...
          const linked = await chunkCQNPMJJ3_cjs.handleOAuthUserInfo(ctx, {
            userInfo: {
