...
var chunkDVHD6IW3_cjs = require('./chunk-DVHD6IW3.cjs');
...
var init = async (options) => {
  const adapter = await chunkDVHD6IW3_cjs.getAdapter(options);
  ...
  const tables = chunkDVHD6IW3_cjs.getAuthTables(options);
  const providers = Object.keys(options.socialProviders || {}).map((key) => {
    ...
    return chunkNU4SXYJ5_cjs.socialProviders[key](
      value
      // TODO: fix this
    );
  }).filter((x) => x !== null);
  ...
  const ctx = {
    ...
    newSession: null,
    adapter,
    internalAdapter: chunkDVHD6IW3_cjs.createInternalAdapter(adapter, {
      options,
      hooks: options.databaseHooks ? [options.databaseHooks] : [],
      generateId: generateIdFunc
    }),
    createAuthCookie: chunkV6HE2MP7_cjs.createCookieGetter(options)
  };
...
function runPluginInit(ctx) {
  ...
  context.internalAdapter = chunkDVHD6IW3_cjs.createInternalAdapter(ctx.adapter, {
    options,
...
var betterAuth = (options) => {
  ...
  const { api } = chunkCQNPMJJ3_cjs.getEndpoints(authContext, options);
  ...
  return {
    ...
    $ERROR_CODES: {
      ...errorCodes,
      ...chunkCQNPMJJ3_cjs.BASE_ERROR_CODES
    }
  };
};
