...
var createInternalAdapter = (adapter, ctx) => {
  ...
  return {
      ...
      if (secondaryStorage) {
        ...
        for (const session of validSessions) {
          ...
          if (sessionStringified) {
            ...
            const parsedSession = chunkB5UCLTPZ_cjs.parseSessionOutput(ctx.options, {
              ...s.session,
    ...
    createSession: async (userId, request, dontRememberMe, override) => {
      ...
      const data = {
        ipAddress: request ? chunkB5UCLTPZ_cjs.getIp(request, ctx.options) || "" : "",
    ...
    findSession: async (token) => {
      if (secondaryStorage) {
        ...
        const s = JSON.parse(sessionStringified);
        const parsedSession2 = chunkB5UCLTPZ_cjs.parseSessionOutput(ctx.options, {
          ...s.session,
          expiresAt: new Date(s.session.expiresAt),
          createdAt: new Date(s.session.createdAt),
          updatedAt: new Date(s.session.updatedAt)
        });
        const parsedUser2 = chunkB5UCLTPZ_cjs.parseUserOutput(ctx.options, {
          ...s.user,
      ...
      const parsedSession = chunkB5UCLTPZ_cjs.parseSessionOutput(ctx.options, session);
      const parsedUser = chunkB5UCLTPZ_cjs.parseUserOutput(ctx.options, user);
      return {
