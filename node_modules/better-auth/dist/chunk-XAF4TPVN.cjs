...
var multiSession = (options) => {
  ...
  return {
    id: "multi-session",
    endpoints: {
      listDeviceSessions: chunkCQNPMJJ3_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const cookies = Object.fromEntries(chunkV6HE2MP7_cjs.parseCookies(cookieHeader));
      ...
      setActiveSession: chunkCQNPMJJ3_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          await chunkV6HE2MP7_cjs.setSessionCookie(ctx, session);
          return ctx.json(session);
        }
      ),
      revokeDeviceSession: chunkCQNPMJJ3_cjs.createAuthEndpoint(
          ...
          if (cookieHeader) {
            ...
            if (sessionTokens.length > 0) {
              ...
              if (validSessions.length > 0) {
                ...
                await chunkV6HE2MP7_cjs.setSessionCookie(ctx, nextSession);
              } else {
      ...
      after: [
        {
          ...
          handler: chunkCQNPMJJ3_cjs.createAuthMiddleware(async (ctx) => {
            const headers = ctx.context.returned instanceof chunkCQNPMJJ3_cjs.APIError ? ctx.context.returned.headers : ctx.responseHeader;
            const cookieString = headers.get("set-cookie");
            if (!cookieString) return;
            const setCookies = chunkV6HE2MP7_cjs.parseSetCookieHeader(cookieString);
            const sessionCookieConfig = ctx.context.authCookies.sessionToken;
            const sessionToken = setCookies.get(
              sessionCookieConfig.name
            )?.value;
            if (!sessionToken) return;
            const cookies = chunkV6HE2MP7_cjs.parseCookies(ctx.headers?.get("cookie") || "");
