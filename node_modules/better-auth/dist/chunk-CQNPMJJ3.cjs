...
var getSession = () => createAuthEndpoint(
  ...
  async (ctx) => {
    try {
      ...
      await chunkV6HE2MP7_cjs.setCookieCache(ctx, session);
      return ctx.json(
        session
      );
    } catch (error2) {
...
var signInSocial = createAuthEndpoint(
  ...
  async (c) => {
    ...
    if (c.body.idToken) {
      ...
      await chunkV6HE2MP7_cjs.setSessionCookie(c, data.data);
      return c.json({
        redirect: false,
...
var callbackOAuth = createAuthEndpoint(
  ...
  async (c) => {
    ...
    await chunkV6HE2MP7_cjs.setSessionCookie(c, {
      session,
      user
    });
    let toRedirectTo;
...
var updateUser = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalFields = chunkB5UCLTPZ_cjs.parseUserInput(
      ctx.context.options,
      rest,
      "update"
    );
...
var signUpEmail = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalData = chunkB5UCLTPZ_cjs.parseUserInput(
      ctx.context.options,
      additionalFields
    );
    let createdUser;
...
async function onRequestRateLimit(req, ctx) {
  ...
  const key = chunkB5UCLTPZ_cjs.getIp(req, ctx.options) + path;
